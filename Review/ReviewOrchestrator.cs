...
// Pipeline driver for `imp review`. Orchestrates:
...
{
    public static async Task<ReviewOutcome?> RunAsync(
        ...
        CancellationToken ct = default)
    {
        ...
        Console.Error.WriteLine($"[review] window: {window.Description}");
        if (window.WarningMessage is not null)
            Console.Error.WriteLine($"[review] warn: {window.WarningMessage}");

        var touched = FileEnumerator.Enumerate(repoRoot, window);
        Console.Error.WriteLine($"[review] {touched.Count} touched files (after filter)");
        ...
        if (touched.Count == 0 || window.IsEmpty)
        {
            ...
            return new ReviewOutcome(emptyPath, WatermarkAdvanced: true, QwenRunCount: 0, SkippedRunCount: 0);
        }

        var plan = AxisPlanner.Plan(touched);
        Console.Error.WriteLine($"[review] axis plan: {plan.Files.Count} files with axes, {plan.EstimatedRunCount} total runs estimated");
        ...
        // --- Pre-pass ---
        Console.Error.WriteLine("[review] pre-pass: building fresh worktree at HEAD…");
        PrePassResult prepass;
        try { prepass = PrePass.Run(repoRoot, window); }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"[review] pre-pass failed: {ex.GetType().Name}: {ex.Message}");
        ...
        Console.Error.WriteLine($"[review] pre-pass: {prepass.AllDiagnostics.Count} diagnostics, {prepass.FormatViolations.Count} format violations");
        ...
        foreach (var (filePlan, axis) in AxisPlanner.DispatchOrder(plan))
        {
            ...
            {
                ReviewAxis.BugScan => "review-bug",
                ReviewAxis.SimplifyComments => "review-simplify-comments",
            ...
            runCount++;
            Console.Error.WriteLine($"[review] {runCount}/{plan.EstimatedRunCount} {modeName} on {file.RelativePath}");
            ...
            try
            {
                ...
                var finding = ExtractFindings(resultJson, axisLabel: axis == ReviewAxis.BugScan ? "bug-scan" : "simplify-comments");
                qwenFindings.AddRange(finding);
            }
            catch (Exception ex)
            {
                Console.Error.WriteLine($"[review] axis run failed ({modeName} on {file.RelativePath}): {ex.GetType().Name}: {ex.Message}");
                skipped++;
            }
        }
        ...
        if (plan.RunUntestedAxis && runCount < maxRuns)
        {
            try
            {
                ...
                runCount++;
                Console.Error.WriteLine($"[review] {runCount}/{plan.EstimatedRunCount} review-untested (cross-cutting)");
                var resultJson = await ResearchRunner.RunAsync(
                    config: config,
            ...
            {
                Console.Error.WriteLine($"[review] untested axis failed: {ex.GetType().Name}: {ex.Message}");
                skipped++;
            }
        }
        ...
        Console.Error.WriteLine($"[review] confidence filter: kept {keptQwen.Count + keptUntested.Count}, dropped {droppedByConfidence}");
        ...
        Console.Error.WriteLine($"[review] report: {reportPath}");
        ...
        if (fullCompletion)
        {
            Watermark.Write(repoRoot, window.HeadSha);
            advanced = true;
            Console.Error.WriteLine($"[review] watermark advanced to {window.HeadSha[..8]}");
        }
        else
        {
            Console.Error.WriteLine("[review] watermark NOT advanced (partial completion)");
        }

        return new ReviewOutcome(reportPath, advanced, runCount, skipped);
    }
    ...
    static TaskDescriptor BuildPerFileDescriptor(
        string repoRoot, ReviewWindow window, TouchedFile file, ReviewAxis axis, PrePassResult prepass)
    {
    ...
    static TaskDescriptor BuildUntestedDescriptor(
        string repoRoot, ReviewWindow window, IReadOnlyList<TouchedFile> touched)
    {
        var bg = new StringBuilder();
    ...
    static string GetDiffForPath(string repoRoot, ReviewWindow window, string relPath)
    {
    ...
    static int CountCommits(string repoRoot, ReviewWindow window)
    {
    ...
    {
        var reviewsDir = Path.Combine(repoRoot, "imp", "reviews");
    ...
    static void PrintDryRun(ReviewWindow window, IReadOnlyList<TouchedFile> touched, ReviewPlan plan, int? maxRuns)
    {
        Console.WriteLine($"# Review dry-run");
