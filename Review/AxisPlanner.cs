...
public sealed record FilePlan(
    TouchedFile File,
    IReadOnlyList<ReviewAxis> Axes);

public sealed record ReviewPlan(
...
public static class AxisPlanner
{
    ...
    public static ReviewPlan Plan(IReadOnlyList<TouchedFile> touched)
    {
        ...
        foreach (var f in touched)
        {
            ...
            if (f.IsProductionCode &&
                ...
                axes.Add(ReviewAxis.SimplifyComments);
        ...
        return new ReviewPlan(files, runUntested, total);
    }
    ...
    public static IEnumerable<(FilePlan File, ReviewAxis Axis)> DispatchOrder(ReviewPlan plan)
    {
        foreach (var fp in plan.Files.OrderByDescending(f => f.File.DiffLineCount))
        {
            if (fp.Axes.Contains(ReviewAxis.SimplifyComments))
                yield return (fp, ReviewAxis.SimplifyComments);
            if (fp.Axes.Contains(ReviewAxis.BugScan))
                yield return (fp, ReviewAxis.BugScan);
        }
    }
}
